//______________1_____________Inc/Dec
...
//______________2_____________Timer
...
document.getElementById('start').addEventListener('click', (e) =>{
    ...
    if(time_inp.value == '')
    {
        countdown.innerHTML = 'nhap lai di!';
    }
    else{
        time_inp.value = '';
...
let data_length = quote_data.length;
let quoteT = document.getElementById('text');
let quoteA = document.getElementById('author');
document.getElementById('quote_btn').addEventListener('click', ()=>{
    let index = Math.floor(Math.random() * data_length);
    quoteT.innerHTML = quote_data[index]['quoteText'];
    quoteA.innerHTML = quote_data[index]['quoteAuthor'];
});
//______________4_____________

let addprj = document.getElementById('addprj');
let addtsk = document.getElementById('addtsk');
let addtime = document.getElementById('addtime');
let btn = document.getElementById('addarow_btn');
let i = 4;
var id_ud;
function clr_input()
{
...
{
    id_ud = n;
    let td_tag = document.getElementById(id_ud).getElementsByTagName('td')
    addprj.value = td_tag[0].innerHTML;
    addtsk.value = td_tag[1].innerHTML;
    addtime.value = td_tag[2].innerHTML;
    btn.innerHTML = 'Update';
}
btn.addEventListener('click', ()=>{
    if(btn.innerHTML == 'Add'){
        document.getElementById('ts_tbody').insertAdjacentHTML('beforeend', `<tr id = ${++i}><td>${addprj.value}</td><td>${addtsk.value}</td><td>${addtime.value}</td><td><button onClick = "rmLine(${i})">X</button><button onClick = "updLine(${i})">U</button></td></tr>`);
        clr_input();
    }
    else{
        let td_tag = document.getElementById(id_ud).getElementsByTagName('td');
        td_tag[0].innerHTML = addprj.value;
        td_tag[1].innerHTML = addtsk.value;
        td_tag[2].innerHTML = addtime.value;
        clr_input();
    }
});
